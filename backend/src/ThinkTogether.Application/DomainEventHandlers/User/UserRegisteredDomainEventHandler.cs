...
namespace ThinkTogether.Application.DomainEventHandlers.User
{
    public class UserRegisteredDomainEventHandler : INotificationHandler<UserRegisteredDomainEvent>
    {
        ...
        public async Task Handle(UserRegisteredDomainEvent notification, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogInformation("Processing user registration event for user {UserId}", notification.UserId);
            ...
            catch (OperationCanceledException ex)
            {
                ...
                throw new OperationCanceledException($"Email sending operation was cancelled for user {notification.UserId}", ex);
            }
            catch (Exception ex)
            {
