...
import { FloatingStationery } from './floating-stationery'
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4 py-10 text-foreground/70">
      ...
      <div className={cn('relative z-10 mx-auto flex w-full flex-col items-center gap-6', hasSideContent ? 'max-w-6xl' : 'max-w-md')}>
        ...
        >
          <div className="pointer-events-none absolute -top-24 right-10 hidden h-44 w-44 rounded-full bg-primary/20 blur-3xl sm:block" />
          <div className="pointer-events-none absolute -bottom-28 left-12 hidden h-48 w-48 rounded-full bg-secondary/20 blur-3xl sm:block" />
