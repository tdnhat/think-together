...
export function SignupForm({ className, onGoogleSignup, ...props }: SignupFormProps) {
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [showPassword, setShowPassword] = useState(false)
  const [showConfirmPassword, setShowConfirmPassword] = useState(false)
  const { register: registerUser } = useAuth()

  const useFormReturn = useForm<SignupFormValues>({
  ...
  return (
    <div className={cn("flex flex-col gap-6", className)} {...props}>
          ...
          <Form {...useFormReturn}>
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="grid gap-6">
                <Button variant="outline" className="w-full" type="button" onClick={onGoogleSignup}>
      ...
      <div className="text-balance text-center text-xs text-muted-foreground [&_a]:underline [&_a]:underline-offset-4 [&_a]:hover:text-primary  ">
        Bằng cách nhấp vào tạo tài khoản, bạn đồng ý với <a href="#">Điều khoản dịch vụ</a>{" "}
